...
function Notify(text)
	exports["notify"]:Notify("Theme Park", text, 5000, "info") -- if you get error in this line its because you dont use our notify system buy it here https://rtx.tebex.io/package/5402098 or you can use some other notify system just replace this notify line with your notify system
	--exports["mythic_notify"]:SendAlert("inform", text, 5000)
end

function AddBumperKey(vehicle)
	TriggerEvent("vehiclekeys:client:SetOwner", plate, rtxbumper)
...
if not IsDuplicityVersion() then
	...
	function RegisterKeyMapping(command, name, input, key)
		CreateThread(
			function()
				_key(command, '[WaAmusment] '..name, input, key)
			end
		)
	end
end