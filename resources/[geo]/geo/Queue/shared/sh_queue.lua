...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s was prioritized and placed %d/%d in queue", tmp.name, _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s was placed %d/%d in queue with prio: %s", tmp.name, _pos, queueCount, Queue:IsPriority(ids) or (src == "debug" and math_random(0, 15))))
    end
...
function playerConnect(name, setKickReason, deferrals, res,source)
    local src = source
    ...
    _Queue.Priority[res.license] = res.whitelist
    deferrals.defer()
