...
window.addEventListener('message', function(event) {
  if (event.data.type == 'hudtoggle') {
    doHide = event.data.bool;
  }
  if (event.data.type == 'fontsize') {
    $('.chat-messages').css('font-size', event.data.size + 'vh')
  }
})
...
window.APP = {
  ...
  mounted() {
    post('http://geo-chat/loaded', JSON.stringify({}));
  ...
  methods: {
    ...
    send(e) {
      if(this.message !== '') {
        post('http://geo-chat/chatResult', JSON.stringify({
    ...
    hideInput(canceled = false) {
      if (canceled) {
        post('http://geo-chat/chatResult', JSON.stringify({ canceled }));
      }
      this.message = '';
...
window.addEventListener('message', (event) => {
  if (event.data.type == 'SetSize') {
    var $chatSize = $(".chat-messages")
    $chatSize.css("font-size", event.data.size + "vh");
  }
});

window.addEventListener('message', (event) => {
  if (event.data.type == 'SetOpacity') {
    var $chatSize = $(".chat-window")
    if (event.data.visible){
      $chatSize.css("opacity", 1.0);
    }
    else {
      $chatSize.css("opacity", 0.0);
    }
  }
});