...
Jobs:RegisterRanking('GoPostal', ranks)
...
AddEventHandler('GoPostal:StartJob', function()
    local source = source
    ...
    if not objList[char.id] then
        objList[char.id] = Jobs.Fetch('GoPostal', char.id)
    ...
    if pool[char.id].LastJob <= time then
        ...
        TriggerClientEvent('Chat.Message', source, '[GoPostal]', ('%s minutes and %s seconds until next available job'):format(since.minutes, since.seconds), 'job')
        return
    end

    if objList[char.id]:GetHours() == 0 or GetUser(source).data.settings.jobinfo then
        TriggerClientEvent('Shared.Notif', source, [[
            The boxes are in the back of your van, take them to your destination then use them and bring them
                up to the door to deliver the package and continue 
        ]], 10000)
    end
    ...
    TriggerClientEvent('Help', source, 4)
    TriggerClientEvent('GoPostal:StartJob', source, pool[char.id].OnJob, jobList[pool[char.id].CurrentID])
end)
...
AddEventHandler('GoPostal:QuitJob', function() 
    ...
    if objList[char.id] then
        ...
        TriggerClientEvent('GoPostal:QuitJob', source, pool[char.id].OnJob)
    end
end)

RegisterNetEvent('FillGoPostal')
AddEventHandler('FillGoPostal', function(plate, model, class)
    TriggerEvent('Inventory:Fetch', 'Vehicle', plate)
    Wait(100)
    exports['geo-inventory']:AddItem('Vehicle', plate, 'box', 100)
end)

RegisterNetEvent('GoPotal:Next')
AddEventHandler('GoPotal:Next', function(pos)
    local source = source
    local char = GetCharacter(source)
    if pool[char.id].OnJob then
        if Vdist4(pos, jobList[pool[char.id].CurrentID][1]) <= 20.0 then
            if exports['geo-inventory']:RemoveItem('Player', source, 'box', 1) then
                objList[char.id]:AddPay(900)
                pool[char.id].CurrentID = Random(#jobList)
                TriggerClientEvent('GoPostal:NextJob', source, jobList[pool[char.id].CurrentID])
            end
        end
    end
end)

RegisterNetEvent('GoPostal:PayMe')
AddEventHandler('GoPostal:PayMe', function()
    local source = source
    local char = GetCharacter(source)

    if pool[char.id] then
        if objList[char.id]:Pay(source, 0) then
            TriggerClientEvent('GoPostal:GetPay', source, objList[char.id].pay)
        end
    end
end)

RegisterNetEvent('GoPostal:GetPay')
AddEventHandler('GoPostal:GetPay', function()
    local source = source
    local char = GetCharacter(source)
    ...
    TriggerClientEvent('GoPostal:GetPay', source, objList[char.id].pay)
    TriggerClientEvent('GoPostal:InformRank', source, objList[char.id]:GetRank())
end)

RegisterNetEvent('GoPostal:Promotion')
AddEventHandler('GoPostal:Promotion', function()
    local source = source
    local char = GetCharacter(source)
    ...
    if objList[char.id]:IsNextRank() then
        ...
        if mth <= 0 then
            ...
            TriggerClientEvent('Chat.Message', source, '[GoPostal]', ('You have been promoted to a %s'):format(objList[char.id]:GetRank()), 'job')
            TriggerClientEvent('GoPostal:InformRank', source, objList[char.id]:GetRank())
        else
            local num = string.format('%.2f', math.abs(mth / 60))
            TriggerClientEvent('Chat.Message', source, '[GoPostal]', ('%s hours until your next promotion'):format(num), 'job')
        end
    end
end)