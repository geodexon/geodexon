...
function tryTakeChips(source,amount)
    ...
    if char.data.casinochips then
        if char.data.casinochips - amount >= 0 then
            char.data.casinochips = char.data.casinochips - amount
            SetData(source, 'casinochips', char.data.casinochips)
            return true
        end
    end
end
...
AddEventHandler("Blackjack:hitBlackjack",function(gameId,nextCardCount, double)
    local source = source
    if double then
        if blackjackGameData[gameId][source][3] == nil and tryTakeChips(source, blackjackGameData[gameId][source][1]) then
            blackjackGameData[gameId][source][1] = blackjackGameData[gameId][source][1] * 2
            blackjackGameData[gameId][source][3] = true
        end
    end
...
RegisterNetEvent('BuyChips', function(amount)
    local source = source
    if RateLimit('BuyChips'..source, 1000) then
        amount = math.abs(math.floor(amount))
        if exports['geo-inventory']:RemoveItem('Player', source, 'dollar', amount) then
            local char = GetCharacter(source)
            if char.data.casinochips == nil then char.data.casinochips = 0 end
            char.data.casinochips = char.data.casinochips + amount
            SetData(source, 'casinochips', char.data.casinochips)
            TriggerClientEvent('Shared.Notif', source, 'You now have '..char.data.casinochips..' chips', 5000)
        end
    end
end)

RegisterNetEvent('Chips.CashOut', function()
    local source = source
    if RateLimit('BuyChips'..source, 1000) then
        local char = GetCharacter(source)
        if not char.data.casinochips then return end
        if exports['geo-inventory']:AddItem('Player', source, 'dollar', char.data.casinochips) then
            char.data.casinochips = 0
            SetData(source, 'casinochips', char.data.casinochips)
        end
    end
end)
