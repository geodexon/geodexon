...
function chairIdToTableId(chairId)
    ...
    if chairId <= 19 then return 4 end  
    if chairId <= 23 then return 5 end  
end
...
Citizen.CreateThread(function()
    ...
    for i=0,#blackjackTables,1 do
        ...
        dealerEntity = CreatePed(26,dealerModel,blackjackTables[i].dealerPos.x,blackjackTables[i].dealerPos.y,blackjackTables[i].dealerPos.z,blackjackTables[i].dealerHeading,false,true)
        ...
        SetEntityCoordsNoOffset(dealerEntity, blackjackTables[i].dealerPos.x,blackjackTables[i].dealerPos.y,blackjackTables[i].dealerPos.z, 0,0,1)
        SetEntityHeading(dealerEntity, blackjackTables[i].dealerHeading)
        SetPedCanBeTargetted(dealerEntity, false)
...
AddEventHandler('AtCasino', function()
    while closeToCasino do 
...
AddEventHandler('AtCasino', function()
    local ped = exports['geo-interface']:InterfacePed({
        ...
        position = vector4(965.53, 24.48, 81.01, 326.08),
        title =  'Casino Chips',
        event = 'CasinoChips'
    })

    local id = exports['geo-interface']:GetPed(ped)
    setBlackjackDealerClothes(0, id)
    while closeToCasino do
        Wait(500)
    end
...
AddEventHandler('CasinoChips', function()
    local menu = {
        {title = 'Get Chips', event = 'Chips.Buy'},
        {title = 'Cash Out', serverevent = 'Chips.CashOut'},
    }

    RunMenu(menu)
end)

AddEventHandler('Chips.Buy', function()
    local chips = Shared.TextInput('Amount of Chips To Buy')
    if tonumber(chips) then
        TriggerServerEvent('BuyChips', tonumber(chips))
    end
end)
...
AddEventHandler('AtCasino', function()
    ...
    while closeToCasino do 
        if drawCurrentHand then 
            ...
            if hidden then
                hidden = false
                _int = Shared.Interact(string.format('Your Hand: %s | Dealer Hand: %s', currentHand, dealersHand) or 'unk')
            end

            _int.update(string.format('Your Hand: %s | Dealer Hand: %s', currentHand, dealersHand))
    ...
    while closeToCasino do 
        if waitingForBetState then
            ...
            if IsControlJustPressed(0, 201) then --Submit bet [Enter]
                ...
                TriggerServerEvent("Blackjack:setBlackjackBet",globalGameId,blackjackTables[chairIdToTableId(closestChair)].bet,closestChair)
...
AddEventHandler('AtCasino', function()
    local _int
    while closeToCasino do
        if closestChair ~= -1 and closestChairDist < 2 and (not sittingAtBlackjackTable) then 
            local tableID = chairIdToTableId(closestChair)
            local amount = blackjackTables[tableID].bet

            _int = Shared.Interact('[E] Play Blackjack $'..comma_value(amount), nil, (MyCharacter.data.casinochips or 0) >= amount) or _int
            while tableID == chairIdToTableId(closestChair) do
                if IsControlJustPressed(0, 38) then --Sit down [E]
                    --print("calling goToBlackjackSeat with chairID: " .. tostring(closestChair))
                    if blackjackTableData[closestChair] == false and (MyCharacter.data.casinochips or 0) >= amount then
    ...
    while closeToCasino do
        ...
        if waitingForStandOrHitState and sittingAtBlackjackTable and blackjackGameInProgress then 
            if hasDoubled then
                ...
                if IsControlJustPressed(0, 38) then --double
                    if MyCharacter.data.casinochips >= blackjackTables[chairIdToTableId(closestChair)].bet and (not hasDoubled) then 
                        waitingForStandOrHitState = false
                        TriggerServerEvent("Blackjack:hitBlackjack",globalGameId,globalNextCardCount, true)
                        hasDoubled = true
                        drawTimerBar = false
                        standOrHitThisRound = true
                        requestCard()
...
Citizen.CreateThread(function()
    while true do 
        ...
        for k,v in pairs(blackjackTables) do
            blackjackTables[k].distance = #(playerCoords-blackjackTables[k].tablePos)
            if blackjackTables[k].distance < 100.0 then
                isClose = true
                if not closeToCasino then
                    closeToCasino = true
                    TriggerEvent('AtCasino')
                    break
                end
            end
        end

        if not isClose and closeToCasino then
            closeToCasino = false
        end
    ...
    while closeToCasino do
        ...
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                ...
                DrawAdvancedNativeText(0.991, 0.935, 0.005, 0.0028, 0.29, "CHIPS:", 255, 255, 255, 255, 0, 0)
                DrawAdvancedNativeText(1.041, 0.928, 0.005, 0.0028, 0.464, tostring(MyCharacter.data.casinochips), 255, 255, 255, 255, 0, 0)
